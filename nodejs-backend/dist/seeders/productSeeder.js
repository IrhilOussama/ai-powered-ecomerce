...
const imageNames = fs.readdirSync(IMAGES_FOLDER_PATH).filter(file => fs.statSync(path.join(IMAGES_FOLDER_PATH, file)).isFile());
// Function to generate a random product
function generateProduct(categoryIds, image) {
    return {
...
async function seedProducts(imagesNames) {
    try {
